...
for other_session in other_sessions:
    ...
    other_spans = [[a for a in x["spans"] if (all(k in a for k in ["start", "end", "label"]) and a["label"] in all_labels)] for x in
                   other_session_subset]
    gold_spans = [[a for a in x["spans"] if (all(k in a for k in ["start", "end", "label"]) and a["label"] in all_labels)] for x in
                  gold_session_subset]

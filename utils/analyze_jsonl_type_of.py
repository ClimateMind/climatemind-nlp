						...
						if relation["child_span"]["label"] == "base":
							...
							if head_span_label:
								if entity_label != base_entity: #ensure base_entity doesn't get added twice
									...
									if head_span_label == "type_of": #if the label is for type_of then add special type_of id (includes doc id, sentence id, span start, span stop)
										type_of_id = document_id+"_"+sentence_id+"_"+str(head_span_start)+"_"+str(head_span_end)
										base_entity_dict[dict_key]["type_of_id"].append(type_of_id)
